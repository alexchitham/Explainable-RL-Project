...
class sverl_saliency_map(xrl_method):
    ...
    def __init__(self, agent_obs, rendered_states, q_values, shapley_func_weights_dir, device="cpu"):
        ...
        self.shapley_val_network = ShapleyEstimator(self.agent_obs.shape, self.num_actions * self.num_pixels * self.num_frames)
...
def mask_states(batch_of_states, masks, device, mask_value=-100):    
    ...
    batch_of_states = batch_of_states.to(dtype=torch.float32, device=device)

    # Turn mask tensor from 1's and 0's, to True's and False's
    masks = masks.to(dtype=torch.bool, device=device)

    # Create the full mask tensor containing all mask values
    full_mask = torch.full_like(batch_of_states, mask_value, device=device)

    # Mask out the pixels specified in the mask
    masked_states = torch.where(masks, batch_of_states, full_mask)
...
def train_char_val_network(model, optimiser, steady_states, agent, device, shapley_sample=True, batch_size=128, num_batches=60003):
    ...
    for batch in range(num_batches):
        ...
        with torch.no_grad():
            true_values = (get_action_probabilities(agent, batch_of_states)).to(device) # Shape: [B, A]
        ...
        if batch % 20000 == 0:
            torch.save(model.state_dict(), "characteristic_func_weights/step" + str(batch))
            most_recent_weights = "characteristic_func_weights/step" + str(batch)
    ...
    with open("char_val_loss.csv", mode="w", newline="") as file:
...
def train_shapley_val_network(model, char_val_network, optimiser, steady_states, num_actions, device, shapley_sample=True, batch_size=128, num_batches=1000):
    # Shape Guide: A = actions, B = batch (128), F = frames, H = height, W = width
    ...
    for batch in range(num_batches):
        ...
        # Periodically save the model's weights
        if batch % 10000 == 0:
            torch.save(model.state_dict(), "shapley_func_weights/step" + str(batch))
            most_recent_weights = "shapley_func_weights/step" + str(batch)
    ...
    # Write the loss data to a CSV file
    with open("shap_val_loss.csv", mode="w", newline="") as file:
        writer = csv.writer(file)
...
def analyse_steady_state(steady_states, char_val_network, agent):
    ...
    for i in range(0, 30, 2):
        ...
        with torch.no_grad():
            true_values = (get_action_probabilities(agent, states))
...
if __name__ == "__main__":    
    ...
    agent = load_dqn_agent("dqn_highway_norm_200K.zip", device=device)
    steady_states = (torch.load("steady_states200k_1.pt", weights_only=True))
    ...
    char_func_weights = train_char_val_network(char_val_network, optimiser_char_val, steady_states, agent, device, num_batches=200005)
    ...
    shap_func_weights = train_shapley_val_network(shap_val_network, char_val_network, optimiser_shap_val, steady_states, num_actions, device, num_batches=100005)


    # ---------------------------------------------------------
    
    # char_val_network.load_state_dict(torch.load("characteristic_func_weights/step60000_3", weights_only=True, map_location=torch.device(device)))
    # char_val_network.eval()
