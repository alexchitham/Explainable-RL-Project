...
class sarfa_saliency_map:
    ...
    def __init__(self, agent_obs, rendered_state, q_values):
        ...
        self.agent_obs_norm = [(self.agent_obs[i]).astype(np.float32) / 255.0 for i in range(self.num_frames)]
        self.rendered_state = rendered_state
        self.num_pixels = self.map.shape[0] * self.map.shape[1]
        ...
        self.p_rem = [self.exp_q_values_no_best[i] / np.sum(self.exp_q_values_no_best) for i in range(self.num_actions - 1)]

        # Blur the whole image
        self.blur_sigma = 5.0
        self.blurred_whole_state = [cv2.GaussianBlur(agent_obs[i], (0, 0), self.blur_sigma) for i in range(self.num_frames)]
    ...
    def calculate_kl_divergence(self, exp_blurred_q_values):
        ...
        kl_divergence = sum(np.multiply(blurred_p_rem, np.log(np.divide(blurred_p_rem, self.p_rem))))
    ...
    def perturb_state(self, pixel):
        ...
        for row in range(mask.shape[0]):
            for col in range(mask.shape[1]):
                ...
                mask[row,col] = np.exp(-((((row - pixel[0]) ** 2) + ((col - pixel[1]) ** 2)) / (2 * (sigma ** 2))))

        # Normalise the mask to [0,1]
        mask = mask / mask.max()

        # Apply the localised blur and scale back up to [0,255]
        perturbed_image = [(self.agent_obs_norm[i] * (1 - mask)) + (self.blurred_whole_state[i] * mask) for i in range(self.num_frames)]
        perturbed_image = [(perturbed_image[i] * 255).astype(np.uint8) for i in range(self.num_frames)]
...
if __name__ == "__main__":
    ...
    agent = load_dqn_agent("dqn_highway_500K")
    env = create_vec_env()
